...
if not exist data\overlay-bypass.conf (
  powershell -NoProfile -Command "$cfg=Get-Content -Raw 'data/restream.json' | ConvertFrom-Json; $overlays=$cfg.overlays; if (-not $overlays) { $overlays=@(); if ($cfg.overlay) { $overlays=@($cfg.overlay) } }; $active=$false; foreach ($o in $overlays) { if ($o -and $o.enabled -and $o.image_file) { $active=$true; break } }; $forceTranscode=$true; if ($cfg.PSObject.Properties.Name -contains 'force_transcode') { $forceTranscode=[bool]$cfg.force_transcode }; if ($active -or $forceTranscode) { '# overlay pipeline active' } else { 'push rtmp://127.0.0.1/live/stream;' }" > data\overlay-bypass.conf
)
